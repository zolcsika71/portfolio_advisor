"""Copy-on-write candidate construction for official MNB HUFONIA evidence."""
...
HUFONIA_EVIDENCE_MIGRATION_REVISION = "MILESTONE_11C_PHASE_D_MNB_HUFONIA_V1"
_REFERENCE_TABLES = (
...
class HufoniaCandidateResult:
    """Safety evidence for one populated disposable HUFONIA database candidate."""
    ...
    sofr_before_fingerprint: str
    sofr_after_fingerprint: str
    schema_contract_fingerprint: str
    import_result: HufoniaImportResult
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            ...
            "sofr_after_fingerprint": self.sofr_after_fingerprint,
            "sofr_before_fingerprint": self.sofr_before_fingerprint,
...
def build_hufonia_evidence_candidate(
    *,
    source: Path,
    candidate: Path,
...
) -> HufoniaCandidateResult:
    """Copy installed provenance-v2 data and add only one validated HUFONIA bundle."""
    if source.is_symlink() or candidate.is_symlink():
        raise HufoniaError("HUFONIA candidate database paths must not be symlinks")
    source = source.resolve()
    candidate = candidate.resolve()
    if not source.is_file() or source == candidate or candidate.exists():
        raise HufoniaError("HUFONIA candidate requires an existing source and absent distinct target")
    ...
    if source_reference_counts != {
        ...
        raise HufoniaError(
            "HUFONIA candidate source does not contain the exact Phase C ESTR+SOFR scope"
        )
    ...
    try:
        ...
        if (
            ...
            or benchmark_stored_row_fingerprint(candidate, "SOFR") != sofr_before
        ):
            raise HufoniaError("disposable HUFONIA candidate differs from its source before import")
        import_result = import_hufonia_evidence(
            target=candidate,
            repository_root=repository_root,
        ...
        if import_result.reused:
            raise HufoniaError("new HUFONIA candidate unexpectedly reused existing HUFONIA evidence")
        validate_hufonia_database(
            target=candidate,
            repository_root=repository_root,
            raw_artifact=raw_artifact,
        ...
        sofr_after = benchmark_stored_row_fingerprint(candidate, "SOFR")
        if (
            candidate_logical != source_logical
            or candidate_base_counts != source_base_counts
            or estr_after != estr_before
            or sofr_after != sofr_before
        ):
            raise HufoniaError(
                "HUFONIA candidate changed pre-existing logical, ESTR, or SOFR evidence"
            )
        ...
        expected["reference_rate_observation"] += HUFONIA_EXPECTED_OBSERVATION_COUNT
        if candidate_reference_counts != expected:
            raise HufoniaError("HUFONIA candidate row-count changes differ from the approved import")
        ...
        if candidate_schema != source_schema:
            raise HufoniaError("HUFONIA candidate changed the provenance-v2 schema contract")
        if _sha256(source) != source_sha256:
            raise HufoniaError("installed source changed during HUFONIA candidate construction")
        return HufoniaCandidateResult(
            migration_revision=HUFONIA_EVIDENCE_MIGRATION_REVISION,
            ...
            sofr_before_fingerprint=sofr_before,
            sofr_after_fingerprint=sofr_after,
