"""Dry-run or explicitly apply retained official NAV evidence to schema-v3."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = integrate_historical_nav(nav_source=args.source, target=args.target, apply=args.apply)
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(json.dumps({**asdict(result), "mode": "APPLY" if args.apply else "DRY_RUN", "cutover_status": "NOT_AUTHORIZED"}, indent=2, sort_keys=True) + "\n")
    return 0
