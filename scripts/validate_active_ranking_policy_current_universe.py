"""Validate the active ranking policy against the current local portfolio universe."""
...
DEFAULT_OUTPUT = Path("data/audit/active_ranking_policy_current_universe_validation.json")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        payload = build_active_policy_validation(
    ...
    except (ActivePolicyValidationError, ValueError, RuntimeError) as error:
        print(f"Active policy validation failed: {error}", file=sys.stderr)
        return 2
    print(f"Active ranking policy: {payload['validation_status']}")
