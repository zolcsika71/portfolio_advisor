"""Build a disposable schema-v3 reference-rate candidate; never install it."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = build_reference_rate_schema_candidate(
            source=arguments.source,
            candidate=arguments.candidate,
        )
    except (ReferenceRateMigrationError, OSError, ValueError) as error:
        print(f"Reference-rate candidate migration failed: {error}", file=sys.stderr)
        return 2
